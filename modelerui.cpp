...
void ModelerUserInterface::updateChoice(ChoiceProperty* prop, Fl_Group* parent, void* v) {
	for (int i = 0; i < parent->children(); i++) {
		...
		button->value(((intptr_t)button->user_data() == prop->getValue()) ? 1 : 0);
	}
}
...
void ModelerUserInterface::CurveTreeCallback(Fl_Tree* tree, void* p) {
	...
	case FL_TREE_REASON_SELECTED:
		ModelerUserInterface::getInstance()->pickCurve((intptr_t)item->user_data(), true);
		break;
	case FL_TREE_REASON_DESELECTED:
		ModelerUserInterface::getInstance()->pickCurve((intptr_t)item->user_data(), false);
		break;
	}
}
...
void ModelerUserInterface::setMovieLength() {
	...
	snprintf(lastLength, 10, "%f", graph->endTime());
	do {
